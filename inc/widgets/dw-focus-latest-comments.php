...
class WP_Widget_Dw_Recent_Comments extends WP_Widget {
	...
	function __construct() {
		...
		$widget_ops = array('classname' => 'widget_dw_recent_comments', 'description' => __( '最新评论小工具', 'dw_focus' ) );
		parent::__construct('dw-recent-comments', __('DW Focus: 最新评论', 'dw_focus' ), $widget_ops);
	...
	function widget( $args, $instance ) {
		...
		if ( isset( $cache[ $args['widget_id'] ] ) ) {
 		...
 		$output = '';
		$title = apply_filters( 'widget_title', empty( $instance['title'] ) ? __( '最新评论', 'dw_focus' ) : $instance['title'], $instance, $this->id_base );
		...
		if ( $comments ) {
			...
			foreach ( (array) $comments as $comment) {
				...
				sprintf(_x('<div class="comment-info"> <span class="comment-date">%1$s</span><span class="comment-title">文章：%2$s</span><span class="comment-author">评论：%3$s</span></div>', 'widgets', 'dw-focus'),
					get_comment_time( 'c' ),
