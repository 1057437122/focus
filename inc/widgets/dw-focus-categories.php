...
class dw_focus_categories_Widget extends WP_Widget {
    ...
    function dw_focus_categories_Widget() {
        $widget_ops = array( 'classname' => 'dw_focus_categories news-category', 'description' => __('显示分类的文章', 'dw_focus') );
        $this->WP_Widget( 'dw_focus_news_by_category', 'DW Focus: 分类文章', $widget_ops );
    }
    ...
    ?>
        <p>
            <label for="<?php echo $this->get_field_id('news-block-category-number'); ?>"><?php _e('显示的文章数','dw_focus'); ?>
        ...
        <div class="meta-info">
            <p>
                <label for="<?php echo $this->get_field_id('meta'); ?>" ><input type="checkbox" name="<?php echo $this->get_field_name('meta') ?>" id="<?php echo $this->get_field_id('meta'); ?>" <?php checked(true, $instance['meta']); ?> class="recent-post-meta-info" >&nbsp;&nbsp;<?php  _e('在文章显示文章元素信息','dw-focus') ?> </label>
            </p>
            <p> --
                <label for="<?php echo $this->get_field_id('date'); ?>" ><input type="checkbox" <?php disabled( false,  $instance['meta'] ); ?> name="<?php echo $this->get_field_name('date') ?>" id="<?php echo $this->get_field_id('date'); ?>" <?php checked(true, $instance['date']); ?> class="submeta-info" >&nbsp;&nbsp;<?php  _e('显示发表日期','dw-focus') ?> </label>
            </p>
            <p> --
                <label for="<?php echo $this->get_field_id('author'); ?>" ><input type="checkbox" <?php disabled( false, $instance['meta'] ); ?> name="<?php echo $this->get_field_name('author') ?>" id="<?php echo $this->get_field_id('author'); ?>" <?php checked(true, $instance['author']); ?> class="submeta-info" >&nbsp;&nbsp;<?php  _e('显示作者信息','dw-focus') ?> </label>
            </p>
            <p> --
                <label for="<?php echo $this->get_field_id('cat'); ?>" ><input type="checkbox" <?php disabled( false,  $instance['meta'] ); ?> name="<?php echo $this->get_field_name('cat') ?>" id="<?php echo $this->get_field_id('cat'); ?>" <?php checked(true, $instance['cat']); ?> class="submeta-info" >&nbsp;&nbsp;<?php  _e('显示分类信息','dw-focus') ?> </label>
            </p>
        </div>
        <p><?php _e('选择分类块的样式','dw_focus'); ?></p>
        <p><label><input <?php checked($instance['news-block-style'], 'full'); ?> type="radio" name="<?php echo $this->get_field_name('news-block-style') ?>" id="<?php echo $this->get_field_id('news-block-style') ?>" value="full" class="dw-focus-category-display-type"> <?php _e('全宽','dw_focus') ?></label><br>
            <label> <input <?php checked($instance['news-block-style'], '3cols'); ?> type="radio" name="<?php echo $this->get_field_name('news-block-style') ?>" id="<?php echo $this->get_field_id('news-block-style') ?>" value="3cols" class="dw-focus-category-display-type"> <?php _e('3 栏','dw_focus') ?></label><br>
        </p>
        <?php 
            $class_extend = '';
            if( 'full' == $instance['news-block-style'] ){
        ...
        <?php echo '<strong> 第一栏 </strong>'; ?>
        </div>
    <?php
        echo '<p>'.__('选择要显示的分类','dw_focus').'</p>';
      ...
      //Get child categories
        ...
        <?php  if( !empty($childs) ){ ?>
         <ul class="child-category hidden-phone">
            <li><a href="" class="tab_title active" data-catid="<?php  echo $category_id; ?>" href="<?php echo $category_link ?>"><?php _e('所有','dw_focus') ?></a></li>
        ...
        <select name="child-category-select-box" id="child-category-for-category-<?php echo $category_id; ?>" class="child-category child-category-select visible-phone">
            <option value="<?php echo $category_id; ?>"><?php _e('-- 所有 --','dw-focus'); ?></option>
            <?php foreach ($childs as $child) { ?>
        ...
        if( $posts->have_posts() ){
            ...
            <div class="span6">
                <article <?php post_class($post_class); ?>>
                    ...
                    <div class="entry-thumbnail">
                        <a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( '阅读： %s', 'dw_focus' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_post_thumbnail('thumbnail'); ?></a>
                    </div>
                    <?php } ?>
                    ...
                    <h2 class="entry-title">
                        <a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( '阅读： %s', 'dw_focus' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a>
                    </h2>
            ...
            <div class="span6">
                ...
                ?>
                    <li><h2 class="entry-title"><a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( '阅读： %s', 'dw_focus' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark"><?php the_title(); ?></a></h2></li>
                <?php } ?>
                </ul>
            </div>
        <?php } else { 
    ...
    function dw_focus_template_1col($args, $number, $category_id, $instance){
        ...
        <article <?php post_class($class); ?>>
            <?php if( has_post_thumbnail(get_the_ID()) ){ ?>
            <div class="entry-thumbnail">
                <a href="<?php the_permalink(); ?>" title="<?php echo esc_attr( sprintf( __( '阅读： %s', 'dw_focus' ), the_title_attribute( 'echo=0' ) ) ); ?>" rel="bookmark">
