...
function dw_focus_content_nav( $nav_id ) {
	...
	<nav role="navigation" id="<?php echo $nav_id; ?>" class="<?php echo $nav_class; ?>">
		<h1 class="assistive-text"><?php _e( '文章导航', 'dw_focus' ); ?></h1>

	<?php if ( is_single() ) : // navigation links for single posts ?>

		<?php previous_post_link( '<div class="nav-previous">%link</div>', '<span class="meta-nav">' . _x( '&larr;', '上一篇文章', 'dw_focus' ) . '</span> %title' ); ?>
		<?php next_post_link( '<div class="nav-next">%link</div>', '%title <span class="meta-nav">' . _x( '&rarr;', '下一篇文章', 'dw_focus' ) . '</span>' ); ?>

	<?php elseif ( $wp_query->max_num_pages > 1 && ( is_home() || is_archive() || is_search() ) ) : // navigation links for home, archive, and search pages ?>

		<?php if ( get_next_posts_link() ) : ?>
		<div class="nav-previous"><?php next_posts_link( __( '<span class="meta-nav">&larr;</span> 较早的文章', 'dw_focus' ) ); ?></div>
		<?php endif; ?>

		<?php if ( get_previous_posts_link() ) : ?>
		<div class="nav-next"><?php previous_posts_link( __( '较新的文章 <span class="meta-nav">&rarr;</span>', 'dw_focus' ) ); ?></div>
		<?php endif; ?>
...
function dw_focus_comment( $comment, $args, $depth ) {
	...
	<li class="post pingback">
		<p><?php _e( '引用:', 'dw_focus' ); ?> <?php comment_author_link(); ?><?php edit_comment_link( __( '(编辑)', 'dw_focus' ), ' ' ); ?></p>
	<?php
			break;
		default :
		...
		<article id="comment-<?php comment_ID(); ?>" class="comment">
			<header class="comment-meta comment-author vcard">
				<?php
					...
					printf( '<a href="%1$s"><time datetime="%2$s">%3$s</time></a> 发表:',
				...
				?>
				<?php edit_comment_link( __( '编辑', 'dw_focus' ) ); ?>
			</header><!-- .comment-meta -->

			<?php if ( '0' == $comment->comment_approved ) : ?>
				<p class="comment-awaiting-moderation"><?php _e( '您的评论正在等待审核。', 'dw_focus' ); ?></p>
			<?php endif; ?>
			...
			<div class="reply">
				<?php comment_reply_link( array_merge( $args, array( 'reply_text' => __( '回复', 'dw_focus' ), 'after' => ' <span><i class="icon-reply"></i></span>', 'depth' => $depth, 'max_depth' => $args['max_depth'] ) ) ); ?>
			</div><!-- .reply -->
		</article><!-- #comment-## -->
	<?php
			break;
