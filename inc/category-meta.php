...
if( ! function_exists('dw_category_generate_html_add_form_fields') ) { 
    ...
    <div class="form-field">
        <label for="tag-style"><?php _e('分类样式','dw_focus') ?></label>
        <select name="tag-style" id="">
            <option value="none"><?php _e('无','dw_focus') ?></option>
            <?php  
        ...
        <p class="description"><?php _e('为分类选择样式','dw_focus'); ?></p>
    </div>
    <?php
    }
    ...
    ?>
        <tr class="form-field">
            <th scope="row" valign="top">
                <label for="tag-style">
                    <?php _e('分类样式','dw_focus') ?>
                </label>
            </th>
            <td>
                <select name="tag-style" id="tag-style">
                    <option <?php selected( $options['style'], 'none' ); ?> value="none"><?php _e('无','dwqa'); ?></option>
                    <?php  
                    if ($handle = opendir(DW_TEMPLATE_PATH . 'assets/colors/') ) {
                        /* This is the correct way to loop over the directory. */
                        while (false !== ($entry = readdir($handle))) {
